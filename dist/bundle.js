...
var functions = __webpack_require__(33);
...
var Button = function (_PureComponent) {
	...
	_createClass(Button, [{
		...
		value: function render() {
			var jss = _style2.default.Button(this.props.primary, this.state.hover, this.props.style);
...
module.exports = function (isPrimary, isHovered, custom) {
	...
	return Object.assign(base, isPrimary ? primary : {}, isHovered ? hover : {}, custom ? custom : {});
};
...
module.exports = function (width, offset, custom) {
	...
	var pct = [4.66666666667, 13.3333333333, 22, 30.6666666667, 39.3333333333, 48, 56.6666666667, 65.3333333333, 74, 82.6666666667, 91.3333333333, 100];
...
module.exports = {
	Wrapper: function Wrapper(custom) {
		return custom ? custom : {};
	},

	Label: function Label(custom) {
		var base = {
			display: 'block',
			fontWeight: 600,
			marginBottom: '.5rem'
	...
	TextInput: function TextInput(custom) {
		return util.m(inputbase, util.fullWidth, custom ? custom : {});
	},

	Select: function Select(custom) {
		return util.m(inputbase, util.fullWidth, custom ? custom : {});
	},

	TextArea: function TextArea(custom) {
		var base = {
			minHeight: 65,
			paddingTop: 6,
			paddingBottom: 6
...
module.exports = {
	list: function list(isOrdered, isNested, custom) {
	...
	listitem: function listitem(custom) {
		var base = {
			marginBottom: '1rem'
		};
...
var enquire = __webpack_require__(36);
var Variables = __webpack_require__(1);
...
var MediaQuery = {
	...
	registerComponent: function registerComponent(c) {
...
module.exports = {
	Header: function Header(size, custom) {
		var base = {
			fontWeight: 300,
			marginTop: 0,
			marginBottom: '2rem'
		...
		var typeMap = [{ fontSize: '4.0rem', lineHeight: 1.2, letterSpacing: '-.1rem' }, { fontSize: '3.6rem', lineHeight: 1.25, letterSpacing: '-.1rem' }, { fontSize: '3.0rem', lineHeight: 1.3, letterSpacing: '-.1rem' }, { fontSize: '2.4rem', lineHeight: 1.35, letterSpacing: '-.108rem' }, { fontSize: '1.8rem', lineHeight: 1.5, letterSpacing: '-.105rem' }, { fontSize: '1.5rem', lineHeight: 1.6, letterSpacing: 0 }];

		return util.m(base, typeMap[size - 1], custom ? custom : {});
	},

	Paragraph: function Paragraph(custom) {
...
Color.prototype.cssHEX = function () {
    return "#" + (255 * this.r < 16 ? "0" : "") + Math.round(255 * this.r).toString(16) + (255 * this.g < 16 ? "0" : "") + Math.round(255 * this.g).toString(16) + (255 * this.b < 16 ? "0" : "") + Math.round(255 * this.b).toString(16);
};
/** Transparency */
...
Color.prototype.saturate = function (v) {
    if ("string" == typeof v && v.indexOf("%") > -1 && (v = parseInt(v)) != 'NaN') this.s += v / 100;else if ("number" == typeof v) // range 255 
        this.s += v / 255;else throw new Error("error: bad modifier format (percent or number)");
...
Color.prototype.lighten = function (v) {
    if ("string" == typeof v && v.indexOf("%") > -1 && (v = parseInt(v)) != 'NaN') this.l += v / 100;else if ("number" == typeof v) // range 255 
        this.l += v / 255;else throw new Error("error: bad modifier format (percent or number)");
...
Color.prototype.fadein = function (v) {
    if ("string" == typeof v && v.indexOf("%") > -1 && (v = parseInt(v)) != 'NaN') this.a += v / 100;else if ("number" == typeof v) // range 255 
        this.a += v / 255;else throw new Error("error: bad modifier format (percent or number)");
...
Color.prototype.spin = function (v) {
    if ("string" == typeof v && v.indexOf("%") > -1 && (v = parseInt(v)) != 'NaN') this.h += v / 100;else if ("number" == typeof v) // range 360 
        this.h += v / 360;else throw new Error("error: bad modifier format (percent or number)");
...
Color.Sanitizer = {
    RGB: function RGB() {
        ...
        for (var i = 0; i < arguments.length; i++) {
            ...
            if ("string" == typeof c && c.indexOf("%") > -1) {
                ...
                if (c < 0) throw new Error("Bad format");else if (c >= 0 && c < 1) o[i] = c;else if (c >= 1 && c < 256) o[i] = c / 255;else throw new Error("Bad format");
            }
        }
        return o;
    },
    HSL: function HSL() {
        ...
        if (h < 0 || h > 360) throw new Error("Hue out of range (0..360)");else if (("" + h).indexOf(".") > -1 && h > 1 || ("" + h).indexOf(".") == -1) h /= 360;
...
Color.Validator = {
     ...
     */
    checkHEX: function checkHEX(value) {
        if (value.length != 6) throw new Error("Hexa color: bad length");
        value = value.toLowerCase();
        for (i in value) {
            var c = value.charCodeAt(i);
            console.log(c);
            if (!(c >= 49 && c <= 57 || c >= 97 && c <= 102)) throw new Error("Hexa color: out of range for " + value + " at position " + i);
        }
    }
};
...
Color.Convertor = {
    ...
    RGBToHSL: function RGBToHSL() {
        ...
        if (max == min) {
            ...
            switch (max) {
                case r:
                    this.h = (g - b) / d + (g < b ? 6 : 0);break;
                case g:
                    this.h = (b - r) / d + 2;break;
                case b:
                    this.h = (r - g) / d + 4;break;
            }
            this.h /= 6;
        }
    },
